...
# ===== KitchenSimple-Compatible Door Task Classes =====

from robocasa.environments.kitchen.kitchen_simple import KitchenSimple


class DoorTaskSimple(KitchenSimple):
    """
    Base class for KitchenSimple-compatible door manipulation tasks.
    """
...
class OpenSingleDoorSimple(DoorTaskSimple):
    """
    KitchenSimple-compatible single door opening task
    """
    ...
    def _get_obj_cfgs(self):
        """
        Get the object configurations for the door opening task.
        Place one object inside a door fixture and distractors on the counter.
        """
        cfgs = []

        # Find door fixture (cabinet, microwave, etc.)
        door_fixture = None
        counter_fixture = None
        
        for name, fixture in self.fixtures.items():
            if ('cabinet' in name.lower() or 'microwave' in name.lower()) and door_fixture is None:
                door_fixture = fixture
            elif 'counter' in name.lower() and counter_fixture is None:
                counter_fixture = fixture
        
        # Fallback
        if door_fixture is None:
            door_fixture = list(self.fixtures.values())[0]
        if counter_fixture is None:
            counter_fixture = list(self.fixtures.values())[0]

        # Object inside door fixture
        cfgs.append(
            dict(
                name="door_obj",
                obj_groups="all",
                graspable=True,
                microwavable=('microwave' in str(door_fixture).lower()),
                type="object",
                placement=dict(
                    fixture=door_fixture,
                    size=(0.30, 0.30),
                    pos=(None, -1.0),
                ),
            )
...
class CloseSingleDoorSimple(DoorTaskSimple):
    """
    KitchenSimple-compatible single door closing task
    """
    ...
    def _get_obj_cfgs(self):
        """
        Get the object configurations for the door closing task.
        Similar to opening task but door starts open.
        """
        cfgs = []
