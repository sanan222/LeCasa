...
from robocasa.models.fixtures import FixtureType, Counter, Stove, Stovetop, HousingCabinet, SingleCabinet, HingeCabinet, Wall
import robocasa.utils.object_utils as OU


class ManipulateDrawer(KitchenSimple):
    """
...
# ===== KitchenSimple-Compatible Drawer Task Classes =====

from robocasa.environments.kitchen.kitchen_simple import KitchenSimple


class DrawerTaskSimple(KitchenSimple):
    """
    Base class for KitchenSimple-compatible drawer manipulation tasks.
    """

    def __init__(self, behavior="open", *args, **kwargs):
        self.behavior = behavior  # "open" or "close"
        super().__init__(*args, **kwargs)
...
class OpenDrawerSimple(DrawerTaskSimple):
    """
    KitchenSimple-compatible drawer opening task
    """
    ...
    def _get_obj_cfgs(self):
        """
        Get the object configurations for the drawer opening task.
        Place object inside the drawer and distractors on the counter.
        """
        cfgs = []

        # Find drawer and counter fixtures
        drawer_fixture = None
        counter_fixture = None
        
        for name, fixture in self.fixtures.items():
            if 'drawer' in name.lower() and drawer_fixture is None:
                drawer_fixture = fixture
            elif 'counter' in name.lower() and counter_fixture is None:
                counter_fixture = fixture
        
        # Fallback
        if drawer_fixture is None:
            drawer_fixture = list(self.fixtures.values())[0]
        if counter_fixture is None:
            counter_fixture = list(self.fixtures.values())[0]
        ...
        # Distractors on counter
        for i in range(2):  # Fixed number for simplicity
            cfgs.append(
                dict(
                    name=f"distr_counter_{i+1}",
                    obj_groups="all",
...
class CloseDrawerSimple(DrawerTaskSimple):
    """
    KitchenSimple-compatible drawer closing task
    """
    ...
    def _get_obj_cfgs(self):
        """
        Get the object configurations for the drawer closing task.
        Similar to opening task but drawer starts open.
        """
        cfgs = []
