...
from tasks.single_stage_so101.environments.kitchen_microwave_door import (
    MicrowaveDoor as _EnvMicrowaveDoor,
)
from tasks.teleoperator import SO101TeleoperationController


class MicrowaveDoorTeleop(SO101TeleoperationController):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.kitchen_env = None

    def initialize_kitchen_environment(self):
        self.kitchen_env = _EnvMicrowaveDoor(
    ...
    def _position_robot_base(self, kitchen_xml, robot_xml):
        ...
        base.set("pos", f"{mx + 0.2:.4f} {my - 0.6:.4f} {mz - 0.1:.4f}")
    ...
    def _add_overhead_camera(self, kitchen_xml, robot_base_body=None):
        ...
        if robot_base_body is not None and robot_base_body.get("pos"):
            ...
            cam.set("pos", f"{bx + 0.1:.4f} {by - 0.3:.4f} {bz + 1.05:.4f}")
        else:
    ...
    def _add_base_stand(self, kitchen_xml, robot_base_body):
        ...
        if robot_base_body is not None and robot_base_body.get("pos"):
            ...
            stand_x, stand_y, stand_z = bx, by-0.1, bz/2  # 10cm below robot base
        else:
